<script lang="ts">
    ...
    const { form, errors, handleSubmit } = createForm({
        ...
        validationSchema: object().shape({
            midi: array()
                .min(1)
                .of(object({ val: number().typeError('midi must be a number') }))
        }),
        onSubmit: (values) => {
        ...
        <Textfield name={`midi[${i}].val`} label="Midi" bind:value={midi.val}>
            <svelte:fragment slot="helper">
                <HelperText persistent validationMsg
                    >{$errors?.midi[i] && cast($errors.midi[i])?.val}</HelperText
                >
            </svelte:fragment>
                ...
                {#if i !== 0}
                    <Wrapper>
                        <IconButton
                            ...
                            aria-label="Remove midi">remove</IconButton
                        >
                ...
                {#if i === $form.midi.length - 1}
                    <Wrapper>
                        <IconButton
                            ...
                            class="material-icons"
                            aria-label="Add midi">add</IconButton
                        >
